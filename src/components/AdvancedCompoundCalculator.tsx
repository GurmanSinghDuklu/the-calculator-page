...
import { TrendingUp, Download, PiggyBank, Sparkles, Calendar, Plus, X, BarChart3, Target, ArrowRight } from "lucide-react";
...
function compoundFlexible(params: {
  ...
  const { initial, years, annualRate, compounding, dcaAmount = 0, dcaEvery = "monthly",
    annualLumpSum = 0, annualLumpMonth = 1, monthlyLumpSums = [], contributionTiming = "end" } = params;
...
function calculateMarketProjection(initial: number, years: number, annualRate: number,
  dcaAmount: number, dcaEvery: FrequencyType, annualLumpSum: number, monthlyLumpSums: MonthlyLumpSum[]): number[] {
...
export const AdvancedCompoundCalculator = () => {
  ...
  const calculate = () => {
    ...
    const res = compoundFlexible({
      initial: parseFloat(initial) || 0, years: parseFloat(years) || 0,
      annualRate: netRate / 100, compounding, dcaAmount: parseFloat(dcaAmount) || 0,
      dcaEvery, annualLumpSum: parseFloat(annualLumpSum) || 0, annualLumpMonth,
      monthlyLumpSums, contributionTiming: timing,
    });
    setResult(res);
  };
  ...
  const downloadCSV = () => {
    ...
    for (const r of result.schedule) csv += `${r.step},${r.year.toFixed(4)},${r.added.toFixed(2)},${r.interest.toFixed(2)},${r.balance.toFixed(2)}\n`;
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `compound-schedule-${new Date().toISOString().slice(0,10)}.csv`;
      ...
      <div className="bg-[#252323]/80 backdrop-blur-xl border border-white/10 rounded-2xl p-6 md:p-8 shadow-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-end">
            <button onClick={() => setDcaAmount("0")} className="w-full py-3 border border-white/10 font-heading text-xs uppercase tracking-widest text-white/40 hover:text-white hover:border-white/30 transition-colors rounded-lg">
              Clear DCA
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-end">
            <button onClick={() => setAnnualLumpSum("0")} className="w-full py-3 border border-white/10 font-heading text-xs uppercase tracking-widest text-white/40 hover:text-white hover:border-white/30 transition-colors rounded-lg">
              Clear
            </button>
          </div>
        </div>
          ...
          {growthBreakdown && (
            <div className="space-y-3">
              ...
              <div className="relative h-8 rounded-full overflow-hidden bg-white/5">
                <div className="absolute left-0 top-0 h-full bg-blue-500/60 transition-all duration-500" style={{ width: `${growthBreakdown.contributionPercent}%` }} />
                <div className="absolute top-0 h-full transition-all duration-500" style={{ left: `${growthBreakdown.contributionPercent}%`, width: `${growthBreakdown.interestPercent}%`, background: `linear-gradient(90deg, ${ACCENT}, #a78bfa)` }} />
              </div>
          ...
          <div>
            ...
            <div className="bg-black/20 border border-white/10 rounded-xl p-4">
              <ResponsiveContainer width="100%" height={300}>
                <AreaChart
                  data={result.schedule.filter((_, i) => i % Math.max(1, Math.ceil(result.schedule.length / 200)) === 0).map(r => ({ year: r.year, balance: r.balance }))}
                ...
                >
                  ...
                  <YAxis stroke="rgba(255,255,255,0.2)" tick={{ fontSize: 10, fontFamily: "Oswald" }} tickFormatter={v => { const a = Math.abs(v); return a >= 1e6 ? `${sym}${(v/1e6).toFixed(1)}M` : a >= 1e3 ? `${sym}${(v/1e3).toFixed(0)}k` : `${sym}${v}`; }} />
                  <Tooltip contentStyle={{ backgroundColor: "#1C1A1A", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "8px", color: "white", fontFamily: "Oswald" }} formatter={(v: number) => [fmtD(v), "Balance"]} labelFormatter={v => `Year ${Number(v).toFixed(1)}`} />
          ...
          {showMarketComparison && marketComparisonData.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => setShowMarketComparison(false)} className="text-[10px] font-heading uppercase tracking-widest text-white/20 hover:text-white transition-colors">Hide</button>
              </div>
              ...
              <div className="bg-black/20 border border-white/10 rounded-xl p-4">
                <ResponsiveContainer width="100%" height={350}>
                  <LineChart data={marketComparisonData} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                    ...
                    <YAxis stroke="rgba(255,255,255,0.2)" tick={{ fontSize: 10, fontFamily: "Oswald" }} tickFormatter={v => { const a = Math.abs(v); return a >= 1e6 ? `${sym}${(v/1e6).toFixed(1)}M` : a >= 1e3 ? `${sym}${(v/1e3).toFixed(0)}k` : `${sym}${v}`; }} />
                    <Tooltip contentStyle={{ backgroundColor: "#1C1A1A", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "8px", color: "white", fontFamily: "Oswald" }} formatter={(v: number) => fmtD(v)} labelFormatter={v => `Year ${v}`} />
          ...
          {showBreakdown && (
            <div className="overflow-auto max-h-96 border border-white/10 rounded-lg">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {result.schedule.filter((_, i) => i % Math.max(1, Math.ceil(result.schedule.length / 500)) === 0).map(row => (
